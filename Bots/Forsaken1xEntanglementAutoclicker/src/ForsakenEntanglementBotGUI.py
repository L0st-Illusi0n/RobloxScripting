...
class EntanglementBotGUI:
    ...
    def _add_slider(self, parent, label, from_, to, variable, command, description):
        ...
        scale = ttk.Scale(row, from_=from_, to=to, variable=variable, command=lambda _: command())
    ...
    def update_settings(self):
        ...
        bot.SCALE = new_scale
        if self.running.get() and abs(new_scale - self._last_scale) > 1e-6:
            self.log_queue.put("[GUI] Reloading templates for new scale.\n")
            bot.load_templates(self.log_queue.put)
